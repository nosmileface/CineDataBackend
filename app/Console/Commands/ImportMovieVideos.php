...
class ImportMovieVideos extends Command
{
    ...
    public function handle(GetMovieVideosService $getMovieVideosService, MovieRepository $movieRepository): int
    {
        try {
            ...
            Log::channel('import-movie-videos')->info('Синхронизация успешна. Получено видео: ' . $count . '. Затрачено времени: ' . $end - $start . ' секунд(ы).');
        ...
        } catch (\Exception $exception)
        {
            ...
            Log::channel('import-movie-videos')->error('Ошибка синхронизации. Исключение: ' . $exception->getMessage());
